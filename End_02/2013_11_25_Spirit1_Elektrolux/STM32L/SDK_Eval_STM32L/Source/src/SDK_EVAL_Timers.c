...
volatile  extern uint32_t lSystickCounter=0;
...
{  
    if (ACQ_wait_answ_timer > 0)        // waiting for the answer
      ACQ_wait_answ_timer--;
  
  if (ACQ_loop_timer > 0)               // waiting till end of second to start next loop
      ACQ_loop_timer--;
  
    if (ACQ_session_timer > 0)          // waiting till end of current session window
      ACQ_session_timer--;
