...
uint8_t start_buffer[96] = {0xAF, 0x00, 0x00, 0x0A, 0x0A, 0x0A, 0x0A};
      ...
      if(xIrqStatus.IRQ_RX_DATA_READY) // Check the SPIRIT1 RX_DATA_READY IRQ Flag 
        {
          ...
          crc = vectcRxBuff[0];
          for(int i = 1; i < (cRxData - 1); i++)
          {
            crc = crc ^ vectcRxBuff[i];
          }
            if( crc == vectcRxBuff[cRxData - 1])
            {
            ...
            if(xIrqStatus.IRQ_TX_FIFO_ERROR) // just to try if this help !!!
            {
              xTxDoneFlag = SET;
            }
...
void command_transfer(void)
  ...
  //      SpiritPktBasicSetPayloadLength(command_size);
        SpiritSpiWriteLinearFifo(96, command_buffer);
        status = SdkEvalSpiReadRegisters(0xE6, 0x01, &pcBuffer);   //ilosc znakow w TX FIFO
//        SpiritSpiWriteLinearFifo(command_index, command_buffer);
        SpiritCmdStrobeTx();
        bajty=SpiritLinearFifoReadNumElementsTxFifo();
        status = SdkEvalSpiReadRegisters(0xE6, 0x01, &pcBuffer);   //ilosc znakow w TX FIFO
        SdkDelayMs(28);
        status = SdkEvalSpiReadRegisters(0xE6, 0x01, &pcBuffer);   //ilosc znakow w TX FIFO
...
void start_sequence(void)
{
  ...
  SdkDelayMs(20);
  SpiritCmdStrobeFlushTxFifo();
  SpiritSpiWriteLinearFifo(7, start_buffer);
  SpiritCmdStrobeTx(); 
  SdkDelayMs(20);
}
